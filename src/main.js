...
function renderPackCard(pack) {
  ...
  return `<article class="pvx-pack-card ${pack.themeClass} ${canAfford ? "" : "locked"}"><div class="pvx-shop-pack-display" aria-hidden="true"><div class="pvx-shop-foil"><span class="pvx-shop-foil-top"></span><span class="pvx-shop-foil-front"><span class="pvx-shop-foil-constellation"></span><span class="pvx-shop-foil-wash"></span><span class="pvx-shop-foil-copy"><small>${escapeHtml(pack.cardPackName)}</small><b>FOIL<br>EDITION</b><em>${pack.amount} cards</em></span><i class="pvx-shop-foil-mark">${pack.icon}</i></span><span class="pvx-shop-foil-seal"></span></div></div><div class="pvx-pack-choice"><small>${escapeHtml(pack.name)}</small><h2>${escapeHtml(pack.cardPackName)}</h2><button data-action="open-pack" data-pack-id="${pack.id}" ${canAfford ? "" : "disabled"}>${canAfford ? `Buy for ◈ ${pack.cost}` : `Need ${pack.cost - gameState.coins} coins`} <span>→</span></button></div></article>`;
}
