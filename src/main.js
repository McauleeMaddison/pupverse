...
function renderPackCard(pack) {
  ...
  const coverCard = cards.find((card) => card.pack === pack.cardPackName) || cards[0];
  return `<article class="pvx-pack-card ${pack.themeClass} ${canAfford ? "" : "locked"}"><div class="pvx-shop-pack-display" aria-hidden="true"><div class="pvx-shop-foil"><span class="pvx-shop-foil-top"></span><span class="pvx-shop-foil-front"><img src="${escapeHtml(coverCard.frontImage)}" alt="" loading="lazy"><span class="pvx-shop-foil-wash"></span><span class="pvx-shop-foil-copy"><small>${escapeHtml(pack.cardPackName)}</small><b>FOIL<br>EDITION</b><em>${pack.amount} cards</em></span><i class="pvx-shop-foil-mark">${pack.icon}</i></span><span class="pvx-shop-foil-seal"></span></div></div><span class="pvx-pack-type">${escapeHtml(pack.name)}</span><h2>${escapeHtml(pack.cardPackName)}</h2><p>${escapeHtml(pack.description)}</p><div class="pvx-pack-meta"><span>${pack.amount} cards</span><span>◈ ${pack.cost}</span></div><button data-action="open-pack" data-pack-id="${pack.id}" ${canAfford ? "" : "disabled"}>${canAfford ? "Open foil pack" : `Need ${pack.cost - gameState.coins} more coins`}<span>→</span></button></article>`;
}
...
function renderSequentialPackReveal(pack, pulls, walkout) {
  ...
  const progressionAction = allRevealed
    ? `<button class="pvx-primary" data-action="finish-reveal">Open Collection Vault →</button>`
    : revealed
